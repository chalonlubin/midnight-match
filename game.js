...
// const deck = _.shuffle(ITEMS);
let deck = (_.shuffle(ITEMS))
createCards(deck);
startTimer();
...
let moves = 0;
let matchesMade = 0;
let possibleMatches = deck.length / 2;
...
function createCards(deck) {
    ...
    for (let card of deck) {
        ...
        thisCard.classList.add(card, "aCard", "front");
        gameBoard.append(thisCard);
        thisCard.addEventListener("click", flipCard);
    }
}
...
function flipCard(event) {
    ...
    if (!disable) {
        ...
        curCard.removeEventListener("click", flipCard);
        ...
        if (firstSelection === secondSelection) {
            ...
            for (let match of matches) {
                ...
                match.removeEventListener("click", flipCard);
            }
    ...
    setTimeout(() => {
        for (let incorrect of noMatch) {
            ...
            setTimeout(() => {
                ...
                incorrect.addEventListener("click", flipCard);
            }, 500);
            disable = false;
        }
    }, 750);
